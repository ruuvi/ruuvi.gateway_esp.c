...
#define GPIO_IS_VALID_OUTPUT_GPIO(gpio_num) \
    ((GPIO_IS_VALID_GPIO(gpio_num)) \
     && (gpio_num < 34)) /*!< Check whether it can be a valid GPIO number of output mode */
 ...
 */
esp_err_t
gpio_set_drive_capability(gpio_num_t gpio_num, gpio_drive_cap_t strength);
 ...
 */
void
gpio_iomux_in(uint32_t gpio_num, uint32_t signal_idx);
