...
TEST_F(TestJsonRuuvi, json_ruuvi_parse) // NOLINT
{
    ...
    cJSON_Delete(root);
    ASSERT_TRUE(gw_cfg.eth_dhcp);
    ASSERT_EQ(string("192.168.1.1"), gw_cfg.eth_static_ip);
    ASSERT_EQ(string("255.255.255.0"), gw_cfg.eth_netmask);
    ASSERT_EQ(string("192.168.0.1"), gw_cfg.eth_gw);
    ASSERT_EQ(string("8.8.8.8"), gw_cfg.eth_dns1);
    ASSERT_EQ(string("4.4.4.4"), gw_cfg.eth_dns2);
    ASSERT_TRUE(gw_cfg.use_mqtt);
    ASSERT_EQ(string("mqtt.server.org"), gw_cfg.mqtt_server);
    ASSERT_EQ(string("prefix"), gw_cfg.mqtt_prefix);
    ASSERT_EQ(1234, gw_cfg.mqtt_port);
    ASSERT_EQ(string("user123"), gw_cfg.mqtt_user);
    ASSERT_EQ(string("pass123"), gw_cfg.mqtt_pass);
    ASSERT_FALSE(gw_cfg.use_http);
    ASSERT_EQ(string("https://api.ruuvi.com:456/api"), gw_cfg.http_url);
    ASSERT_EQ(string("user567"), gw_cfg.http_user);
    ASSERT_EQ(string("pass567"), gw_cfg.http_pass);
    ASSERT_TRUE(gw_cfg.company_filter);
    ASSERT_EQ(888, gw_cfg.company_id);
...
TEST_F(TestJsonRuuvi, json_ruuvi_parse_http_body) // NOLINT
{
    ...
    ASSERT_FALSE(gw_cfg.eth_dhcp);
    ASSERT_EQ(string(""), gw_cfg.eth_static_ip);
    ASSERT_EQ(string(""), gw_cfg.eth_netmask);
    ASSERT_EQ(string(""), gw_cfg.eth_gw);
    ASSERT_EQ(string(""), gw_cfg.eth_dns1);
    ASSERT_EQ(string(""), gw_cfg.eth_dns2);
    ASSERT_TRUE(gw_cfg.use_mqtt);
    ASSERT_EQ(string("test.mosquitto.org"), gw_cfg.mqtt_server);
    ASSERT_EQ(string("ruuvi/30:AE:A4:02:84:A4"), gw_cfg.mqtt_prefix);
    ASSERT_EQ(1883, gw_cfg.mqtt_port);
    ASSERT_EQ(string(""), gw_cfg.mqtt_user);
    ASSERT_EQ(string(""), gw_cfg.mqtt_pass);
    ASSERT_FALSE(gw_cfg.use_http);
    ASSERT_EQ(string("https://network.ruuvi.com:443/gwapi/v1"), gw_cfg.http_url);
    ASSERT_EQ(string(""), gw_cfg.http_user);
    ASSERT_EQ(string(""), gw_cfg.http_pass);
    ASSERT_TRUE(gw_cfg.company_filter);
    ASSERT_EQ(0, gw_cfg.company_id);
