 ...
 */
esp_err_t
heap_caps_register_failed_alloc_callback(esp_alloc_failed_hook_t callback);
 ...
 */
void*
heap_caps_realloc(void* ptr, size_t size, uint32_t caps);
 ...
 */
void*
heap_caps_aligned_alloc(size_t alignment, size_t size, uint32_t caps);
...
/**
 * @brief Allocate a aligned chunk of memory which has the given capabilities. The initialized value in the memory is
 * set to zero.
 *
 * @param alignment  How the pointer received needs to be aligned
 *                   must be a power of two
 ...
 * tracked per-region. Individual regions' heaps may have reached their "low water marks" at different points in time.
 * However this result still gives a "worst case" indication for all-time minimum free heap.
 *
 * @param caps        Bitwise OR of MALLOC_CAP_* flags indicating the type
 *                    of memory
 *
 ...
 */
size_t
heap_caps_get_minimum_free_size(uint32_t caps);
 ...
 */
void
heap_caps_print_heap_info(uint32_t caps);
 ...
 */
bool
heap_caps_check_integrity_all(bool print_errors);
 ...
 */
bool
heap_caps_check_integrity(uint32_t caps, bool print_errors);
 ...
 */
bool
heap_caps_check_integrity_addr(intptr_t addr, bool print_errors);
 ...
 */
void
heap_caps_malloc_extmem_enable(size_t limit);
 ...
 */
void*
heap_caps_malloc_prefer(size_t size, size_t num, ...);
 ...
 */
void*
heap_caps_realloc_prefer(void* ptr, size_t size, size_t num, ...);
 ...
 */
void*
heap_caps_calloc_prefer(size_t n, size_t size, size_t num, ...);
