...
public:
    ...
    explicit TestEvent(const TestEventType_e eventType) : eventType(eventType)
    {
    }
};
...
{
public:
    TestEventEspTimerInit() : TestEvent(TestEventType_EspTimerInit)
    {
    }
};
...
public:
    ...
    TestEventLedcSetFadeWithTime(ledc_mode_t speed_mode, ledc_channel_t channel, uint32_t target_duty, int max_fade_time_ms) :
