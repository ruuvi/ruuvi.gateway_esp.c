...
#define URL_ENCODE_BASE               (16)
#define URL_ENCODE_NUM_CHARS_PER_BYTE (2)
...
url_n_decode_to_str_buf(const char* const p_src, const size_t len, str_buf_t* const p_dst)
{
    ...
    while (('\0' != *p_cur) && ((ptrdiff_t)(p_cur - p_src) < len))
    {
        if ('%' == *p_cur)
        {
            ...
            const char  tmp_buf[URL_ENCODE_NUM_CHARS_PER_BYTE + 1] = { p_cur[0], p_cur[1], '\0' };
            const char* p_end                                      = NULL;
            uint32_t    ch_val = os_str_to_uint32_cptr(tmp_buf, &p_end, URL_ENCODE_BASE);
            if ('\0' != *p_end)
            {
                return false;
            }
