...
typedef struct mqtt_protected_data_t
{
    ...
    tls_shared_buf_mqtts_t*    p_tls_shared_buf_mqtts;
    ...
    void* const                              p_user_context)
{
    ...
    p_cli_cfg->path                        = NULL;
    p_cli_cfg->ssl_buf_cfg                 = p_cfg_params->ssl_buf_cfg;
}
...
mqtt_app_start_internal(mqtt_protected_data_t* p_mqtt_data, const ruuvi_gw_cfg_mqtt_t* const p_mqtt_cfg)
{
    ...
    {
        p_mqtt_data->str_buf_client_cert = gw_cfg_storage_read_file_as_string(GW_CFG_STORAGE_SSL_MQTT_CLI_CERT);
        p_mqtt_data->str_buf_client_key  = gw_cfg_storage_read_file_as_string(GW_CFG_STORAGE_SSL_MQTT_CLI_KEY);
    }
    if (p_mqtt_cfg->use_ssl_server_cert)
    {
        p_mqtt_data->str_buf_server_cert_mqtt = gw_cfg_storage_read_file_as_string(GW_CFG_STORAGE_SSL_MQTT_SRV_CERT);
    }
