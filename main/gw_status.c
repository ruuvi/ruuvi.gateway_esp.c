...
#define GW_STATUS_CONFIGURE_BUTTON_PRESSED   (1U << 12U)
#define GW_STATUS_WAITING_AUTO_CFG_BY_WPS    (1U << 13U)
    ...
    const bool flag_wait)
{
    ...
    {
        LOG_INFO(
            "Wait until relaying mode is changed: notify_http=%d, notify_mqtt=%d",
            (int)flag_notify_change_for_http,
            (int)flag_notify_change_for_mqtt);
        gw_status_wait_after_relaying_mode_changed(flag_notify_change_for_http, flag_notify_change_for_mqtt);
        LOG_INFO("Wake up: Relaying mode has changed");
    }
}
...
{
    xEventGroupSetBits(g_p_ev_grp_status_bits, GW_STATUS_CONFIGURE_BUTTON_PRESSED);
}

void
gw_status_clear_configure_button_pressed(void)
{
    xEventGroupClearBits(g_p_ev_grp_status_bits, GW_STATUS_CONFIGURE_BUTTON_PRESSED);
}

bool
gw_status_get_configure_button_pressed(void)
{
    if (0 != (xEventGroupGetBits(g_p_ev_grp_status_bits) & GW_STATUS_CONFIGURE_BUTTON_PRESSED))
    {
        return true;
    }
    return false;
...
{
    xEventGroupSetBits(g_p_ev_grp_status_bits, GW_STATUS_WAITING_AUTO_CFG_BY_WPS);
}

void
gw_status_clear_waiting_auto_cfg_by_wps(void)
{
    xEventGroupClearBits(g_p_ev_grp_status_bits, GW_STATUS_WAITING_AUTO_CFG_BY_WPS);
}

bool
gw_status_is_waiting_auto_cfg_by_wps(void)
{
    if (0 != (xEventGroupGetBits(g_p_ev_grp_status_bits) & GW_STATUS_WAITING_AUTO_CFG_BY_WPS))
    {
        return true;
    }
    return false;
