...
typedef struct runtime_stat_task_info_t
{
    UBaseType_t task_number;
    uint32_t    runtime_counter;
} runtime_stat_task_info_t;

typedef struct runtime_stat_accumulated_task_info_t
{
    char     task_name[configMAX_TASK_NAME_LEN];
    uint32_t min_free_stack_size;
} runtime_stat_accumulated_task_info_t;
...
runtime_stat_update_accumulated_info(const char* const p_task_name, const uint32_t free_stack_size)
{
    ...
    {
        runtime_stat_accumulated_task_info_t* const p_info = &g_runtime_stat_accumulated[i];
...
runtime_stat_for_each_accumulated_info(runtime_stat_cb_t p_cb, void* p_userdata)
{
    ...
    for (uint32_t i = 0; i < (sizeof(g_runtime_stat_accumulated) / sizeof(g_runtime_stat_accumulated[0])); ++i)
    {
        const runtime_stat_accumulated_task_info_t* const p_info = &g_runtime_stat_accumulated[i];
        if ('\0' == p_info->task_name[0])
        {
            continue;
        }
...
static runtime_stat_task_info_idx_t
runtime_stat_find_task_info(
    const runtime_stat_task_info_t* const p_arr_of_tasks,
    const uint32_t                        max_num_tasks,
    const UBaseType_t                     task_num)
{
    for (runtime_stat_task_info_idx_t i = 0; i < max_num_tasks; ++i)
    {
        const runtime_stat_task_info_t* const p_task_info = &p_arr_of_tasks[i];
...
runtime_stat_add_task_info(
    runtime_stat_task_info_t* const p_arr_of_tasks,
    const uint32_t                  max_num_tasks,
    const UBaseType_t               task_num,
    const uint32_t                  runtime_counter)
{
    for (runtime_stat_task_info_idx_t i = 0; i < max_num_tasks; ++i)
    {
        runtime_stat_task_info_t* const p_task_info = &p_arr_of_tasks[i];
...
log_runtime_compare_tasks(const void* const p_task1, const void* const p_task2)
{
    ...
    {
        return (qsort_int_t)p_task_stat1->xTaskNumber - (qsort_int_t)p_task_stat2->xTaskNumber;
    }
    return (qsort_int_t)p_task_stat1->uxBasePriority - (qsort_int_t)p_task_stat2->uxBasePriority;
}
...
{
    runtime_stat_task_info_idx_t task_info_idx = runtime_stat_find_task_info(
        g_tasks_info,
        sizeof(g_tasks_info) / sizeof(*g_tasks_info),
        p_task_status->xTaskNumber);

    runtime_stat_task_info_t* const p_task_info = (task_info_idx < (sizeof(g_tasks_info) / sizeof(*g_tasks_info)))
