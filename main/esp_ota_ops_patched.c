...
#ifdef CONFIG_BOOTLOADER_APP_ANTI_ROLLBACK
static esp_err_t
esp_ota_set_anti_rollback(void)
{
...
static esp_err_t
esp_ota_current_ota_is_workable(bool valid)
{
    esp_ota_select_entry_t otadata[2];
    const esp_partition_t *otadata_partition = read_otadata(otadata);
    if (otadata_partition == NULL)
...
static int
get_last_invalid_otadata(const esp_ota_select_entry_t *two_otadata)
{
...
esp_ota_get_last_invalid_partition()
{
    ...
    if (invalid_otadata != -1 && ota_app_count != 0)
    {
        ...
        const esp_partition_t *invalid_partition = esp_partition_find_first(
            ESP_PARTITION_TYPE_APP,
...
esp_ota_erase_last_boot_app_partition(void)
{
    ...
    if (otadata[inactive_otadata].ota_seq == UINT32_MAX
    ...
    const esp_partition_t *last_boot_app_partition_from_otadata = esp_partition_find_first(
        ESP_PARTITION_TYPE_APP,
        ESP_PARTITION_SUBTYPE_APP_OTA_MIN + ota_slot,
        NULL);
    ...
    esp_err_t err = esp_partition_erase_range(
        ...
        0,
        last_boot_app_partition_from_otadata->size);
    if (err != ESP_OK)
    {
        return err;
    }
