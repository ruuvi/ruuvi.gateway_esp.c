                                   ...
                                   size_t pms_offset)
{
    ...
    if (offset + len_bytes > ssl_out_content_len) {
    ...
    if ((ret = mbedtls_pk_encrypt(peer_pk,
                                  ...
                                  ssl_out_content_len - offset - len_bytes,
...
ecdh_calc_secret:
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_PSK) {
        ...
        *p++ = MBEDTLS_BYTE_0(ssl->conf->psk_identity_len);
        ...
        unsigned char *end = ssl->out_msg + ssl_out_content_len;
        size_t own_pubkey_max_len = (size_t) (end - own_pubkey);
...
static int ssl_parse_new_session_ticket(mbedtls_ssl_context *ssl)
{
    ...
    if (ticket_len > sizeof(ssl->session_negotiate->ticket.buf)) {
        ESP_LOGW(TAG, "%s: session ticket is too large (%u bytes) - ignore it", __func__, (unsigned)ticket_len);
        ssl->session_negotiate->ticket_len = 0;
        return 0;
    }
