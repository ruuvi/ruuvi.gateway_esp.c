...
* **Evaluation**:
  ...
  * M2M local endpoints require **Access Control** and **Privilege Separation** to prevent
...
* **Evaluation**:
  ...
  * `SecComMech-WebUI-Session`: Mitigates local HTTP (Port 80) plain-text risks via
...
| `SecComMech-Firmware-Signature-Verification` | RSA-3072-PSS with SHA-256                                                     | **SOGIS v1.1 / NIST SP 800-57:** RSA $\ge 3000$ bits with PSS padding provides robust 128-bit signature strength.                                      |         128-bit          | **Best Practice** |
...
* **Evaluation**:
  ...
  * RSA-3072-PSS signature validation mathematically eliminates PKCS#1 v1.5 padding oracle
    vulnerabilities.
* **Verdict**: **PASS**
...
system boot, Web-UI administrative sessions, cloud telemetry streaming, and OTA update checks to
analyze negotiated protocol parameters.
...
| **Local M2M API** (`SecComMech-LAN-Bearer-Authentication`) | HTTP Authorization Bearer token comparison (`lan_auth_api_key`).                               | Requests to restricted endpoints without valid `Authorization: Bearer <token>` headers are immediately rejected with HTTP 401 Unauthorized.                                                                    |   **PASS**   |

**Assessment Justification**: Functional network traffic analysis demonstrates that the DUT enforces
all cryptographic settings, protocol versions, and cipher suites documented in `IXIT 11-SecComMech`.
Cleartext transmission of sensitive parameters is prevented across all active network interfaces.
...
|:-------------------|:------------------------------------|:--------------------------------------------------------------------------------------------------|:--------:|
| **5.5-1-1 Unit a** | Security Guarantees Appropriateness | Claimed guarantees (Confidentiality, Authenticity, Integrity) match communication use cases.      | **PASS** |
| **5.5-1-1 Unit b** | Mechanism Design Appropriateness    | TLS 1.2/1.3, hybrid ECDH Web-UI encryption, and HMAC signing effectively achieve guarantees.      | **PASS** |
| **5.5-1-1 Unit c** | Best Practice Reference Catalogue   | Primitives (AES-GCM, ECDH P-256, SHA-256/384, RSA-3072-PSS) align with SOGIS and NIST standards.  | **PASS** |
| **5.5-1-1 Unit d** | Feasible Attack Assessment          | AEAD ciphers and nonced authentication eliminate padding oracle and replay attack vectors.        | **PASS** |
| **5.5-1-2 Unit a** | Functional Traffic Verification     | Packet captures confirm runtime enforcement of documented TLS ciphers, ECDH, and HMAC signatures. | **PASS** |
...
AES-GCM, ECDH P-256 key agreement, HMAC-SHA256 payload signing, and RSA-3072-PSS signature
verification) recognized in SOGIS, NIST, and ETSI reference catalogues. Functional network protocol
analysis confirms that all documented cryptographic settings are strictly enforced during operation.
