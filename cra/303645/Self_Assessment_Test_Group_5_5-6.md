...
| **`SecParam-LAN-Bearer-Tokens`**                 | Transmitted over local network Port 80 for M2M REST API access (`SecComMech-LAN-Bearer-Authentication`). | `N/A` (Cleartext Bearer Token in standard HTTP `Authorization` headers).                     | **N/A (Opt-in feature, disabled by default).** Tokens are passed in cleartext HTTP headers. This optional mechanism relies on local network isolation or reverse proxy TLS termination (e.g. NGINX HTTPS proxy) if exposed beyond trusted LAN segments. |   **N/A**    |
...
* **Conceptual Assessment Justification**: All critical security parameters transmitted across
  ...
  requirement for confidentiality in transit. The optional M2M bearer tokens
  (`SecParam-LAN-Bearer-Tokens`)
  are disabled by default and explicitly document cleartext HTTP transmission in `IXIT 10-SecParam`,
  relying on physical/network-layer boundaries or TLS reverse proxies.
...
parameters referencing secure communication mechanisms are transmitted exclusively within encrypted
sessions on the wire and that no cleartext credentials leak during transit.
...
| **M2M API Token Evaluation**          | `SecParam-LAN-Bearer-Tokens` via HTTP Port 80.                                 | Token travels in cleartext HTTP `Authorization` header as declared; feature is confirmed disabled by default and requires explicit user enablement in Web-UI.                                       |   **N/A**    |

**Assessment Justification**: Functional network traffic analysis confirms that the DUT encrypts all
critical security parameters during transmission across all defined secure communication mechanisms.
Sniffing unencrypted HTTP and network sockets verified zero leakage of cleartext Wi-Fi passphrases,
administrative passwords, TLS private keys, or MQTT secrets.
...
|:-------------------|:--------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:--------:|
| **5.5-6-1 Unit a** | Conceptual In-Transit Confidentiality Check | All active CSPs requiring encryption in transit are protected by TLS 1.2/1.3 or ECDH/AES-CBC payload encryption. `SecParam-LAN-Bearer-Tokens` is evaluated as N/A (disabled by default). | **PASS** |
| **5.5-6-2 Unit a** | Functional Network Traffic Verification     | Wireshark packet captures verify zero cleartext leakage of credentials, private keys, or rotated secrets across all secure communication paths.                                          | **PASS** |
...
or application-layer ECDH P-256 / AES-CBC encryption). The local M2M Bearer token interface is
disabled by default and documented as cleartext HTTP for advanced users, relying on LAN isolation or
TLS reverse proxy architectures.
