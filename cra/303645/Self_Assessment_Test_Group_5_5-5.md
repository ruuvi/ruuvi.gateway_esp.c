...
* **Testing Methodology**: The test laboratory executed unauthorized POST requests, invalid
...
|:-------------------------------------|:----------------------------------|:--------------------------------------------------------------------------------------------|:----------------------------------------------------------------------------------------------------------------------------------|:------------:|
| **Factory Default State Protection** | `SoftServ-Local-Management-WebUI` | Send `POST /ruuvi.json` without authenticating using the factory $DEVICEID$ password.       | Server returns HTTP `401 Unauthorized`. Configuration changes are blocked until the unique default credential challenge succeeds. |   **PASS**   |
...
| **5.5-5-2 Unit a** | Functional Authorization Verification      | Functional testing confirms unauthenticated POSTs (HTTP 401) and read-only token writes (HTTP 403) are strictly rejected.  | **PASS** |
...
default and initialized states. Functional testing and network port scanning confirm that
unauthorized configuration changes are prevented and that all logical network interfaces are fully
documented in `IXIT 28-LogIntf`.
