...
| Provision 5.1-2  | M F (b)      |   Y     | Supported via IXIT-1-2. Passwords are hardware-unique (2^64 entropy) and pre-installed at manufacturing.             |
| Provision 5.1-2A | R F (c)      |   Y     | Best practice followed: M2M uses tokens, not passwords. See IXIT-1-8, 1-9.                                           |
| Provision 5.1-3  | M F (d)      |   Y     | Uses ECDH, AES-CBC, and SHA-256. See IXIT-1-2, 1-3, 1-5, 1-8, 1-9.                                                   |
| Provision 5.1-4  | M F (e)      |   Y     | Change mechanism provided via Web-UI/Configuration. Transitions device from IXIT-1-2 to 1-3. Also supports 1-4, 1-5. |
| Provision 5.1-5  | M C F (1, f) |   Y     | Time delays and rate limiting implemented. See IXIT-1-2, 1-3, 1-8, 1-9.                                              |
