...
def scan_cpp(base_path, rel_project_path):
    ...
    if not os.path.isdir(test_project_path):
        print(f'Error: there is no folder: {test_project_path}')
        sys.exit(1)
    dict_test_suites = dict()
    re_line = re.compile('^class\s+([\w\d_]+)\s*:\s*public\s*::testing::Test')
    for dirpath, dirnames, filenames in os.walk(test_project_path):
        for file in filenames:
            if os.path.splitext(file)[1] == '.cpp':
                cpp_file_path = os.path.join(dirpath, file)
                print(f'    Found: {os.path.relpath(cpp_file_path)}')
...
def handle_gtest_results(input_file, base_path, rel_project_path, xml_root):
    ...
    if gtest_xml.tag != 'testsuites':
        print(f'Error: Bad input file: expected "testsuites" section, but found "{gtest_xml.tag}"')
        sys.exit(1)
    ...
    for test_suite in test_suits:
        ...
        if test_suite.tag != 'testsuite':
            print(f'Error: Bad input file: expected "testsuite" section, but found "{test_suite.tag}"')
            sys.exit(1)
        test_suite_name = test_suite.attrib['name']
        print(f'    Test suite name: {test_suite_name}')
        ...
        for test_case in test_cases:
            ...
            if test_case.tag != 'testcase':
                print(f'Error: Bad input file: expected "testcase" section, but found "{test_case.tag}"')
                sys.exit(1)
            ...
            print(f'        Test case: {test_case_name}: {test_case_status}: {test_case_result}')
            ...
            if test_case_status == 'run':
                if test_case_result == 'completed':
                    ...
                    for failure in failures:
                        ...
                        if failure.tag != 'failure':
                            print(f'Error: Bad input file: expected "failure" section, but found "{failure.tag}"')
                            sys.exit(1)
                        failure_message = failure.attrib['message']
                        failure_text = failure.text
                        failure_message_short = failure_message.partition('\n')[0]
                        print(f'            Failure: {failure_message_short}')
                    ...
                    print(f'Error: Unknown test case result in status "run": {test_case_result}')
                    sys.exit(1)
                ...
                if test_case_result == 'suppressed':
                    ...
                    print(f'Error: Unknown test case result in status "notrun": {test_case_result}')
                    sys.exit(1)
            else:
                print(f'Error: Unknown test case status: {test_case_status}')
                sys.exit(1)
    ...
    for dirpath, dirnames, filenames in os.walk(cmake_build_path):
        for file in filenames:
            if file == 'gtestresults.xml':
                ...
                print(f'Found google-test results: {os.path.join(dirpath, file)}')
